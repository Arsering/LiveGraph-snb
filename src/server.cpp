...
#pragma omp parallel for
    ...
    if (positions.back() != size - 1)
        ret.emplace_back(std::string(data + positions.back() + 1, size - positions.back()));
...
public:
    CSVReader() = delete;
    CSVReader(const std::string &path) : cursor_(0)
    {
    ...
    std::string GetNextLine()
    {
        ...
        while (cursor_ < size_)
        {
            ...
            // GBPLOG << start_index << " " << end_index << " " << size_;
...
{
    auto loader = graph->begin_batch_loader();
    CSVReader data_loader(path);
    data_loader.GetNextLine();

    auto data_line = data_loader.GetNextLine();
    while (!data_line.empty())
    {
        auto v = split(data_line, csv_split);
        uint64_t id = std::stoul(v[0]);
        uint64_t vid = loader.new_vertex();
        schema.insertId(id, vid);
        data_line = data_loader.GetNextLine();
    }

    assert(data_loader.GetNextLine().empty());
}
...
#pragma omp parallel for
    ...
    for (size_t i = 1; i < all_persons.size(); i++)
    {
        ...
        auto person_buf = snb::PersonSchema::createPerson(
            ...
            std::chrono::duration_cast<std::chrono::hours>(birthday.time_since_epoch()).count(), emails_v, speaks_v,
            person_v[7], person_v[6],
            std::chrono::duration_cast<std::chrono::milliseconds>(creationDate.time_since_epoch()).count(), place_vid);
        ...
        //        std::cout << person->id << "|" << person->place << "|" << std::string(person->firstName(),
        //        person->firstNameLen()) << "|"
...
#pragma omp parallel for
    ...
    for (size_t i = 1; i < all_places.size(); i++)
    {
        ...
        auto place_buf =
            snb::PlaceSchema::createPlace(std::stoull(place_v[0]), place_v[1], place_v[2], type, isPartOf_vid);
        ...
        //            << std::string(place->url(), place->urlLen()) << "|"  << (int)place->type << "|" <<
        //            place->isPartOf << std::endl;
    }
}
...
#pragma omp parallel for
    ...
    for (size_t i = 1; i < all_orgs.size(); i++)
    {
        ...
        //            << std::string(org->url(), org->urlLen()) << "|"  << (int)org->type << "|" << org->place <<
        //            std::endl;
    }
}

void importPost(snb::MessageSchema &postSchema,
                std::string path,
                snb::PersonSchema &personSchema,
                snb::ForumSchema &forumSchema,
                snb::PlaceSchema &placeSchema)
{
    ...
    std::vector<uint64_t> message_vids(message_vs.size()), creator_vids(message_vs.size()),
        forumid_vids(message_vs.size()), place_vids(message_vs.size());
#pragma omp parallel for
    ...
    for (size_t i = 0; i < message_vs.size(); i++)
    {
        ...
        auto message_buf = snb::MessageSchema::createMessage(
            std::stoull(message_v[0]), message_v[1], creationDateInt, message_v[3], message_v[4], message_v[5],
            message_v[6], creator_vid, forumid_vid, place_vid, (uint64_t)-1, (uint64_t)-1, type);
        ...
        loader.put_edge(creator_vid, (label_t)snb::EdgeSchema::Person2Post_creator, message_vid,
                        std::string_view((char *)&creationDateInt, sizeof(creationDateInt)), true);
...
void importComment(snb::MessageSchema &commentSchema,
                   std::string path,
                   snb::PersonSchema &personSchema,
                   snb::PlaceSchema &placeSchema,
                   snb::MessageSchema &postSchema)
...
#pragma omp parallel for
    ...
    for (size_t i = 0; i < message_vs.size(); i++)
    {
        ...
        auto message_buf = snb::MessageSchema::createMessage(
            std::stoull(message_v[0]), std::string(), creationDateInt, message_v[2], message_v[3], std::string(),
            message_v[4], creator_vid, (uint64_t)-1, place_vid, replyOfPost_vid, replyOfComment_vid, type);

        loader.put_vertex(message_vid, std::string_view(message_buf.data(), message_buf.size()));
        loader.put_edge(creator_vid, (label_t)snb::EdgeSchema::Person2Comment_creator, message_vid,
                        std::string_view((char *)&creationDateInt, sizeof(creationDateInt)), true);
        if (replyOfPost_vid != (uint64_t)-1)
        {
            loader.put_edge(replyOfPost_vid, (label_t)snb::EdgeSchema::Message2Message_down, message_vid,
                            std::string_view((char *)&creationDateInt, sizeof(creationDateInt)), true);
        }
        if (replyOfComment_vid != (uint64_t)-1)
        {
            loader.put_edge(replyOfComment_vid, (label_t)snb::EdgeSchema::Message2Message_down, message_vid,
                            std::string_view((char *)&creationDateInt, sizeof(creationDateInt)), true);
        }
...
#pragma omp parallel for
    ...
    for (size_t i = 1; i < all_tagclasss.size(); i++)
    {
        ...
        auto tagclass_buf = snb::TagClassSchema::createTagClass(std::stoull(tagclass_v[0]), tagclass_v[1],
                                                                tagclass_v[2], isSubclassOf_vid);

        loader.put_vertex(tagclass_vid, std::string_view(tagclass_buf.data(), tagclass_buf.size()));
        if (isSubclassOf_vid != (uint64_t)-1)
        {
            loader.put_edge(isSubclassOf_vid, (label_t)snb::EdgeSchema::TagClass2TagClass_down, tagclass_vid,
                            std::string_view(), true);
        }

        //        const snb::TagClassSchema::TagClass *tagclass = (const
        //        snb::TagClassSchema::TagClass*)tagclass_buf.data(); std::cout << tagclass->id << "|" <<
        //        std::string(tagclass->name(), tagclass->nameLen()) << "|"
        //            << std::string(tagclass->url(), tagclass->urlLen()) << "|"  << tagclass->isSubclassOf <<
        //            std::endl;
    }
}
...
void importRawEdge(
    snb::Schema &xSchema, snb::Schema &ySchema, snb::EdgeSchema x2y, snb::EdgeSchema y2x, std::string path)
{
...
void importDataEdge(snb::Schema &xSchema,
                    snb::Schema &ySchema,
                    snb::EdgeSchema x2y,
                    snb::EdgeSchema y2x,
                    std::string path,
                    ...
                    bool sort)
{
    ...
    all_dataEdges.clear();
    if (sort)
        tbb::parallel_sort(dataEdge_vs.begin(), dataEdge_vs.end(),
                           [](const std::vector<std::string> &a, const std::vector<std::string> &b)
                           { return a[2] < b[2]; });
...
#pragma omp parallel for
    ...
    for (size_t i = 0; i < dataEdge_vs.size(); i++)
    {
        ...
        *(uint64_t *)(buf.data() + sizeof(uint64_t)) = dataEdge_posts[i];
        ...
        loader.put_edge(forum_id, (label_t)snb::EdgeSchema::Forum2Person_member, person_id,
                        std::string_view(buf.data(), buf.size()), true);
        loader.put_edge(person_id, (label_t)snb::EdgeSchema::Person2Forum_member, forum_id,
                        std::string_view(buf.data(), buf.size()), true);
    }
}
...
public:
    ...
    virtual InteractiveIf *getHandler(const ::apache::thrift::TConnectionInfo &connInfo)
    {
        ...
        return new InteractiveHandler(graph, personSchema, placeSchema, orgSchema, postSchema, commentSchema, tagSchema,
...
int main(int argc, char **argv)
{
    ...
    if (graph->get_max_vertex_id() == 0)
    {
...

        {
            ...
            pool.emplace_back(importPost, std::ref(postSchema), dataPath, std::ref(personSchema), std::ref(forumSchema),
                              std::ref(placeSchema));
            pool.emplace_back(importComment, std::ref(commentSchema), dataPath, std::ref(personSchema),
                              std::ref(placeSchema), std::ref(postSchema));
            ...
            pool.emplace_back(importRawEdge, std::ref(personSchema), std::ref(tagSchema), snb::EdgeSchema::Person2Tag,
                              snb::EdgeSchema::Tag2Person, dataPath + snb::personHasInterestPathSuffix);
            pool.emplace_back(importRawEdge, std::ref(postSchema), std::ref(tagSchema), snb::EdgeSchema::Post2Tag,
                              snb::EdgeSchema::Tag2Post, dataPath + snb::postHasTagPathSuffix);
            pool.emplace_back(importRawEdge, std::ref(commentSchema), std::ref(tagSchema), snb::EdgeSchema::Comment2Tag,
                              snb::EdgeSchema::Tag2Comment, dataPath + snb::commentHasTagPathSuffix);
            pool.emplace_back(importRawEdge, std::ref(forumSchema), std::ref(tagSchema), snb::EdgeSchema::Forum2Tag,
        ...
        }
        {
            ...
            //            snb::EdgeSchema::Forum2Person_member, snb::EdgeSchema::Person2Forum_member,
            ...
            pool.emplace_back(importDataEdge, std::ref(personSchema), std::ref(commentSchema),
                              snb::EdgeSchema::Person2Comment_like, snb::EdgeSchema::Comment2Person_like,
                              dataPath + snb::personLikeCommentPathSuffix, DateTimeParser, true);
            pool.emplace_back(importDataEdge, std::ref(personSchema), std::ref(orgSchema),
