...
void InteractiveHandler::query14(std::vector<Query14Response> &_return, const Query14Request &request)
{
    ...
    if (engine.get_vertex_gbp(vid1) == BlockManager::NULLPOINTER)
        return;
    if (engine.get_vertex_gbp(vid2) == BlockManager::NULLPOINTER)
        return;
    auto paths = pairwiseShortestPath_path(engine, vid1, vid2, (label_t)snb::EdgeSchema::Person2Person);
    std::sort(paths.begin(), paths.end(),
              [](const std::pair<double, std::vector<uint64_t>> &a, const std::pair<double, std::vector<uint64_t>> &b)
    ...
    for (auto p : paths)
    {
        ...
        for (auto vid : p.second)
        {
            ...
            if (iter == idx.end())
            {
                iter = idx.emplace(vid, 0).first;
            }
            _return.back().personIdsInPath.emplace_back(iter->first);
            person_vids.emplace_back(iter->first);
        }
    }
    ...
    {
        auto &path = _return[i].personIdsInPath;
        for (auto j = 0; j < path.size(); j++)
        {
            path[j] = person_schemas[person_cursor++].id;
        }
    }
}
#else
    ...
    for (auto p : paths)
    {
        ...
        for (auto vid : p.second)
        {
            ...
            if (iter == idx.end())
            {
                ...
                iter = idx.emplace(vid, person_schema.id).first;
            }
