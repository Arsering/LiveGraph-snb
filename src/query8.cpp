...
void InteractiveHandler::query8(std::vector<Query8Response> &_return, const Query8Request &request)
{
    ...
    {
        edgelist_infos.emplace_back(posts[i], (label_t)snb::EdgeSchema::Message2Message_down, false);
    }
    {
    ...
    {
        edgelist_infos.emplace_back(comments[i], (label_t)snb::EdgeSchema::Message2Message_down, false);
    }
    {
        auto nbrses = engine.get_edges_gbp(edgelist_infos);
        for (size_t i = 0; i < comments.size(); i++)
