...
namespace snb
{
    ...
    public:
        ...
        Schema() : db(nullptr), vindex(nullptr), rvindex(nullptr), nameindex(nullptr), read_options(), write_options()
        {
        }
        Schema(rocksdb::DB *_db,
               rocksdb::ColumnFamilyHandle *_vindex,
               rocksdb::ColumnFamilyHandle *_rvindex,
               rocksdb::ColumnFamilyHandle *_nameindex)
    ...
    public:
        struct Person
        {
            ...
            const char *firstName() const { return data; }
            uint16_t firstNameLen() const { return lastName_offset; }
            const char *lastName() const { return data + lastName_offset; }
            uint16_t lastNameLen() const { return gender_offset - lastName_offset; }
            const char *gender() const { return data + gender_offset; }
            uint16_t genderLen() const { return emails_offset - gender_offset; }
            const char *emails() const { return data + emails_offset; }
            uint16_t emailsLen() const { return speaks_offset - emails_offset; }
            const char *speaks() const { return data + speaks_offset; }
            uint16_t speaksLen() const { return browserUsed_offset - speaks_offset; }
            const char *browserUsed() const { return data + browserUsed_offset; }
            uint16_t browserUsedLen() const { return locationIP_offset - browserUsed_offset; }
            const char *locationIP() const { return data + locationIP_offset; }
            uint16_t locationIPLen() const { return length - locationIP_offset; }
        }; // __attribute__((packed));

        Buffer static createPerson(uint64_t id,
                                   ...
                                   uint64_t place)
        {
            ...
            offset += browserUsed.length();
            person->locationIP_offset = offset;

            memcpy(person->data + offset, locationIP.c_str(), locationIP.length());
            offset += locationIP.length();
            person->length = offset;
    ...
    public:
        struct Place
        {
            ...
            uint16_t nameLen() const { return url_offset; }
            const char *url() const { return data + url_offset; }
            uint16_t urlLen() const { return length - url_offset; }
        }; // __attribute__((packed));
    ...
    public:
        struct Message
        {
            ...
            const char *imageFile() const { return data; }
            uint16_t imageFileLen() const { return locationIP_offset; }
            const char *locationIP() const { return data + locationIP_offset; }
            uint16_t locationIPLen() const { return browserUsed_offset - locationIP_offset; }
            const char *browserUsed() const { return data + browserUsed_offset; }
            uint16_t browserUsedLen() const { return language_offset - browserUsed_offset; }
            const char *language() const { return data + language_offset; }
            uint16_t languageLen() const { return content_offset - language_offset; }
            const char *content() const { return data + content_offset; }
            uint16_t contentLen() const { return length - content_offset; }
        }; // __attribute__((packed));

        Buffer static createMessage(uint64_t id,
    ...
    public:
        ...
        Buffer static createTagClass(uint64_t id, std::string name, std::string url, uint64_t isSubclassOf)
        {
            ...
            offset += name.length();
            tagclass->url_offset = offset;

            memcpy(tagclass->data + offset, url.c_str(), url.length());
            offset += url.length();
            tagclass->length = offset;

            assert(tagclass->length + sizeof(TagClass) == size);
            return buf;
        }

        TagClassSchema() : Schema() {}
        TagClassSchema(rocksdb::DB *_db,
                       rocksdb::ColumnFamilyHandle *_vindex,
                       rocksdb::ColumnFamilyHandle *_rvindex,
                       rocksdb::ColumnFamilyHandle *_nameindex)
            : Schema(_db, _vindex, _rvindex, _nameindex)
    ...
    public:
        struct Forum
        {
            ...
            const char *title() const { return data; }
            uint16_t titleLen() const { return length; }
        }; // __attribute__((packed));
