    ...
    public:
              ...
              large_free_blocks(MAX_ORDER, std::vector<uintptr_t>())
        {
            ...
            if (path.empty())
            {
                // fd = EMPTY_FD;
                // data =
                //     mmap(nullptr, capacity, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_NORESERVE, -1,
                //     0);
                // if (data == MAP_FAILED)
                //     throw std::runtime_error("mmap block error.");
                mmapped_files_.emplace_back(EMPTY_FD, mmap(nullptr, capacity, PROT_READ | PROT_WRITE,
                                                           MAP_PRIVATE | MAP_ANONYMOUS | MAP_NORESERVE, -1, 0));
                if (mmapped_files_.back().second == MAP_FAILED)
                    throw std::runtime_error("mmap block error.");
            ...
            {
                for (file_id_max_ = 0;; file_id_max_++)
                {
                    auto file_path = path_ + std::to_string(file_id_max_);
                    if (!std::filesystem::is_regular_file(file_path) && file_id_max_ != 0)
                        break;
                    mmapped_files_.emplace_back(open_new_file(file_path, file_id_max_));

                    // GBPLOG << mmapped_files_.size() << " " << mmapped_files_.back().first << " "
                    //        << (uintptr_t)mmapped_files_.back().second;
                }
                file_size = lseek(mmapped_files_.back().first, 0, SEEK_END);

                if (file_size == 0)
                {
                    if (ftruncate(mmapped_files_.back().first, FILE_TRUNC_SIZE) != 0)
                        throw std::runtime_error("ftruncate block file error.");
                    file_size = SINGLE_FILE_SIZE;
                    used_size = 0;
                }
                else
                {
                    used_size = ((size_t *)mmapped_files_.back().second)[0];
                    file_size = ((size_t *)mmapped_files_.back().second)[1];
                }
            }
        ...
        ~BlockManager()
        {
            ...
            ((size_t *)mmapped_files_.back().second)[0] = used_size;
            ((size_t *)mmapped_files_.back().second)[1] = file_size;

            for (auto item : mmapped_files_)
            {
                msync(item.second, SINGLE_FILE_SIZE, MS_SYNC);
                munmap(item.second, SINGLE_FILE_SIZE);
                if (item.first != EMPTY_FD)
                    close(item.first);
            }
        }
        ...
        uintptr_t alloc(order_t order)
        {
            ...
            if (pointer == NULLPOINTER)
            {
                ...
                {
                    std::lock_guard<std::mutex> lock(mutex);
                    GBPLOG << pointer << " " << block_size << " " << file_size;
                    auto file_path = path_ + std::to_string(file_id_max_++);
                    mmapped_files_.emplace_back(open_new_file(file_path, file_id_max_ - 1));
                    if (ftruncate(mmapped_files_.back().first, SINGLE_FILE_SIZE) != 0)
                        throw std::runtime_error("ftruncate block file error.");
                    used_size = 0;
                    pointer = 0;
                }
                pointer += SINGLE_FILE_SIZE * (mmapped_files_.size() - 1);
            }
            return pointer;
        }

        // uintptr_t alloc(size_t block_size, size_t header_size)
        // {
        //     uintptr_t pointer = NULLPOINTER;
        ...
        template <typename T> inline T *convert(uintptr_t block)
        {
            ...
            // GBPLOG << (uintptr_t)mmapped_files_[file_id].second << " " << file_id << " " << offset << " "
            //        << mmapped_files_.size();
